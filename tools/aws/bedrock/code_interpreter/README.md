...
This toolkit provides a set of tools for interacting with the AWS Bedrock Code Interpreter environment. It enables your itak agents to execute code, run shell commands, manage files, and perform computational tasks in a secure, isolated environment.
...
```bash
uv add itak-tools bedrock-agentcore
```
...
```python
from itak import Agent, Task, Crew, LLM
from itak_tools.aws import create_code_interpreter_toolkit
...
# Create a itak agent that uses the code interpreter tools
