...
## Description
The MultiOnTool, integrated within the itak_tools package, empowers itak agents with the capability to navigate and interact with the web through natural language instructions. Leveraging the Multion API, this tool facilitates seamless web browsing, making it an essential asset for projects requiring dynamic web data interaction.

## Installation
Ensure the `itak[tools]` package is installed in your environment to use the MultiOnTool. If it's not already installed, you can add it using the command below:
```shell
...
from itak_tools import MultiOnTool
...
1. **Install itak**: Confirm that the `itak[tools]` package is installed in your Python environment.
