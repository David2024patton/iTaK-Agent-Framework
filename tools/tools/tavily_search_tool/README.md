...
The `TavilySearchTool` provides an interface to the Tavily Search API, enabling itak agents to perform comprehensive web searches. It allows for specifying search depth, topics, time ranges, included/excluded domains, and whether to include direct answers, raw content, or images in the results. The tool returns the search results as a JSON string.
...
```shell
pip install 'itak[tools]' tavily-python
```
...
Here's how to initialize and use the `TavilySearchTool` within a itak agent:

```python
import os
from itak import Agent, Task, Crew
from itak_tools import TavilySearchTool
