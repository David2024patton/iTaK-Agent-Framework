...
from itak_tools.tools.rag.types import (
    AddDocumentParams,
    ContentItem,
    RagToolConfig,
    VectorDbConfig,
...
class RagTool(BaseTool):
    ...
    def _ensure_adapter(self) -> Self:
        if isinstance(self.adapter, RagTool._AdapterPlaceholder):
            from itak_tools.adapters.iTaK_rag_adapter import itakRagAdapter

            provider_cfg = self._parse_config(self.config)
            self.adapter = iTaKRagAdapter(
    ...
    def _parse_config(self, config: RagToolConfig) -> Any:
        ...
        if provider not in supported:
            raise ValueError(
                ...
                f"iTaK RAG currently supports: {', '.join(supported)}."
            )
