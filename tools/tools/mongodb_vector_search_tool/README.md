...
## Installation
Install the itak_tools package with MongoDB support by executing the following command in your terminal:

```shell
pip install itak-tools[mongodb]
```

or

```
uv add itak-tools --extra mongodb
```
...
```python
from itak_tools import MongoDBVectorSearchTool
...
```python
from itak_tools import MongoDBVectorSearchConfig, MongoDBVectorSearchTool
...
# Add the text from a set of itak knowledge documents.
texts = []
