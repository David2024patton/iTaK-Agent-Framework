...
The `TavilyExtractorTool` allows itak agents to extract structured content from web pages using the Tavily API. It can process single URLs or lists of URLs and provides options for controlling the extraction depth and including images.
...
Here's how to initialize and use the `TavilyExtractorTool` within a itak agent:

```python
import os
from itak import Agent, Task, Crew
from itak_tools import TavilyExtractorTool
