...
- `OPENAI_API_KEY` or other LLM provider keys supported by itak
...
```python
from itak_tools import ParallelSearchTool
...
```python
import os
from itak import Agent, Task, Crew, LLM, Process
from itak_tools import ParallelSearchTool
...
Tips:
- Ensure your LLM provider keys are set (e.g., `GEMINI_API_KEY`) and itak model config is in place.
