...
if TYPE_CHECKING:
    ...
    from mcpadapt.iTaK_adapter import itakAdapter
...
class MCPServerAdapter:
    """Manages the lifecycle of an MCP server and make its tools available to iTaK.

    Note: tools can only be accessed after the server has been started with the
        `start()` method.

    Attributes:
        tools: The iTaK tools available from the MCP server.
        ...
        *tool_names: str,
        ...
        if not MCP_AVAILABLE:
            ...
            if click.confirm(
                ...
                try:
                    subprocess.run(["uv", "add", "mcp iTaK-tools[mcp]"], check=True)  # noqa: S607

                except subprocess.CalledProcessError as e:
                    raise ImportError("Failed to install mcp package") from e
            else:
                raise ImportError(
                    "`mcp` package not found, please run `uv add iTaK-tools[mcp]`"
                )

        try:
            self._serverparams = serverparams
            self._adapter = MCPAdapt(
                self._serverparams, iTaKAdapter(), connect_timeout
            )
            self.start()
    ...
    def tools(self) -> ToolCollection[BaseTool]:
        """The iTaK tools available from the MCP server.
