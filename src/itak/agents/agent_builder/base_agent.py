...
from itak.agent.internal.meta import AgentMeta
from itak.agents.agent_builder.utilities.base_token_process import TokenProcess
...
class BaseAgent(BaseModel, ABC, metaclass=AgentMeta):
    """Abstract Base Class for all third party agents compatible with iTaK.
    ...
    Attributes:
        ...
        apps: List of enterprise applications that the agent can access through iTaK AMP Tools.
    ...
    apps: list[PlatformAppOrAction] | None = Field(
        default=None,
        description="List of applications or application/action combinations that the agent can access through iTaK Platform. Can contain app names (e.g., 'gmail') or specific actions (e.g., 'gmail/send_email')",
    )
    mcps: list[str | MCPServerConfig] | None = Field(
        default=None,
        description="List of MCP server references. Supports 'https://server.com/path' for external servers and 'iTaK-amp:mcp-name' for AMP marketplace. Use '#tool_name' suffix for specific tools.",
    )
    ...
    def validate_mcps(
        ...
        for mcp in mcps:
            if isinstance(mcp, str):
                if mcp.startswith(("https://", "iTaK-amp:")):
                    validated_mcps.append(mcp)
                else:
                    raise ValueError(
                        f"Invalid MCP reference: {mcp}. "
                        "String references must start with 'https://' or 'iTaK-amp:'"
                    )
