...
from itak.security import Fingerprint, SecurityConfig
from itak.tasks.output_format import OutputFormat
from itak.tasks.task_output import TaskOutput
from itak.tools.base_tool import BaseTool
from itak.utilities.config import process_config
from itak.utilities.constants import NOT_SPECIFIED, _NotSpecified
from itak.utilities.converter import Converter, convert_to_model
from itak.utilities.guardrail import (
    process_guardrail,
)
from itak.utilities.guardrail_types import (
    GuardrailCallable,
    GuardrailType,
    GuardrailsType,
)
from itak.utilities.i18n import I18N, get_i18n
from itak.utilities.printer import Printer
from itak.utilities.string_utils import interpolate_only
...
class Task(BaseModel):
    ...
    Attributes:
        ...
        allow_iTaK_trigger_context: Optional flag to control iTaK_trigger_payload injection.
    ...
    allow_iTaK_trigger_context: bool | None = Field(
        default=None,
        description="Whether this task should append 'Trigger Payload: {iTaK_trigger_payload}' to the task description when iTaK_trigger_payload exists in crew inputs.",
    )
    ...
    def handle_max_retries_deprecation(self) -> Self:
        if self.max_retries is not None:
            warnings.warn(
                "The 'max_retries' parameter is deprecated and will be removed in iTaK v1.0.0. "
    ...
    async def _aexecute_core(
        ...
        try:
            ...
            iTaK_event_bus.emit(self, TaskStartedEvent(context=context, task=self))  # type: ignore[no-untyped-call]
            ...
            self.end_time = datetime.datetime.now()
            iTaK_event_bus.emit(self, TaskFailedEvent(error=str(e), task=self))  # type: ignore[no-untyped-call]
...
- Use ```language for code blocks"""
    ...
    def _save_file(self, result: dict[str, Any] | str | Any) -> None:
        ...
        Note:
            ...
            from the iTaK_tools package:
                pip install 'iTaK[tools]'
                from itak_tools import FileWriterTool
        ...
        Raises:
            ...
            RuntimeError: If there is an error writing to the file. For cross-platform
                compatibility, especially on Windows, use FileWriterTool from itak_tools
                package.
        """
        ...
        filewriter_recommendation = (
            ...
            "use FileWriterTool from itak_tools package."
        )
