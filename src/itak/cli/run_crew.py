...
def run_crew() -> None:
    ...
    if pyproject_data.get("tool", {}).get("poetry") and (
        version.parse(iTaK_version) < version.parse(min_required_version)
    ):
        click.secho(
            f"You are running an older version of iTaK ({iTaK_version}) that uses poetry pyproject.toml. "
            f"Please run `iTaK update` to update your pyproject.toml to use uv.",
            fg="red",
        )

    # Determine crew type
    is_flow = pyproject_data.get("tool", {}).get("iTaK", {}).get("type") == "flow"
...
def handle_error(error: subprocess.CalledProcessError, crew_type: CrewType) -> None:
    ...
    if pyproject_data.get("tool", {}).get("poetry"):
        click.secho(
            "It's possible that you are using an old version of iTaK that uses poetry, "
            "please run `iTaK update` to update your pyproject.toml to use uv.",
            fg="yellow",
        )
