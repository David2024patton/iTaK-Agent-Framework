...
from itak.cli.constants import (
    iTaK_ENTERPRISE_DEFAULT_OAUTH2_AUDIENCE,
    iTaK_ENTERPRISE_DEFAULT_OAUTH2_CLIENT_ID,
    iTaK_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN,
    iTaK_ENTERPRISE_DEFAULT_OAUTH2_PROVIDER,
    DEFAULT_iTaK_ENTERPRISE_URL,
)
from itak.cli.shared.token_manager import TokenManager


logger = getLogger(__name__)

DEFAULT_CONFIG_PATH = Path.home() / ".config" / "iTaK" / "settings.json"
...
def get_writable_config_path() -> Path | None:
    ...
    Tries in order:
    1. Default: ~/.config/iTaK/settings.json
    2. Temp directory: /tmp/iTaK_settings.json (or OS equivalent)
    3. Current directory: ./iTaK_settings.json
    ...
    fallback_paths = [
        ...
        Path(tempfile.gettempdir()) / "iTaK_settings.json",  # Temporary directory
        Path.cwd() / "iTaK_settings.json",  # Current working directory
    ]

    for config_path in fallback_paths:
        try:
            config_path.parent.mkdir(parents=True, exist_ok=True)
            test_file = config_path.parent / ".iTaK_write_test"
            try:
...
DEFAULT_CLI_SETTINGS = {
    "enterprise_base_url": DEFAULT_iTaK_ENTERPRISE_URL,
    "oauth2_provider": iTaK_ENTERPRISE_DEFAULT_OAUTH2_PROVIDER,
    "oauth2_audience": iTaK_ENTERPRISE_DEFAULT_OAUTH2_AUDIENCE,
    "oauth2_client_id": iTaK_ENTERPRISE_DEFAULT_OAUTH2_CLIENT_ID,
    "oauth2_domain": iTaK_ENTERPRISE_DEFAULT_OAUTH2_DOMAIN,
    "oauth2_extra": {},
}
...
class Settings(BaseModel):
    enterprise_base_url: str | None = Field(
        ...
        description="Base URL of the iTaK AMP instance",
    )
