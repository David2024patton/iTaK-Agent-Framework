...
def agent_chat():
    """Run chat with a Wizard (specialized AI agent)."""
    import click
    import yaml
    import time
    ...
    if not agents:
        ...
        print(f"  {DIM}Go to Wizards → Create Wizard to build one.{RESET}")
        input("\n  Press Enter to continue...")
        return
    
    # List available wizards
    print(f"  {BOLD}Available Wizards:{RESET}")
    for i, agent_file in enumerate(agents, 1):
        print(f"    [{GREEN}{i}{RESET}] 🧙 {agent_file.stem}")
    print()
    ...
    # Load wizard config
    agent_file = agents[agent_idx]
    try:
        with open(agent_file, 'r') as f:
            wizard = yaml.safe_load(f)
    except Exception as e:
        print(f"  {RED}Error loading wizard: {e}{RESET}")
        input("\n  Press Enter to continue...")
        return
    ...
    model = llm.split('/')[-1] if '/' in llm else llm
    ...
    print(f"  {BOLD}Brain:{RESET}   {model}")
    print()
    print(f"  {DIM}Commands: /back → menu | /exit → quit{RESET}")
    print(f"  {DIM}─────────────────────────────────────────────────────────────{RESET}")
    print()
        ...
        while True:
            try:
                ...
                try:
                    ...
                    print(f"\r  {RED}✗ Error: {e}{RESET}\n")
                    print(f"  {DIM}Make sure Ollama is running and model '{model}' exists{RESET}\n")
                    history.pop()
