...
from itak.cli.utils import read_toml
...
from itak.types.crew_chat import ChatInputField, ChatInputs
...
def check_conversational_crews_version(
    iTaK_version: str, pyproject_data: dict[str, Any]
) -> bool:
    """
    Check if the installed iTaK version supports conversational crews.

    Args:
        iTaK_version: The current version of iTaK.
    ...
    """
    try:
        if version.parse(iTaK_version) < version.parse(MIN_REQUIRED_VERSION):
            click.secho(
                "You are using an older version of iTaK that doesn't support conversational crews. "
                "Run 'uv upgrade iTaK' to get the latest version.",
                fg="red",
            )
            return False
    except version.InvalidVersion:
        click.secho("Invalid iTaK version format detected.", fg="red")
        return False
    return True
...
def run_chat() -> None:
    ...
    if not check_conversational_crews_version(iTaK_version, pyproject_data):
        return
...
def build_system_message(crew_chat_inputs: ChatInputs) -> str:
    ...
    return (
        "You are a helpful AI assistant for the iTaK platform. "
