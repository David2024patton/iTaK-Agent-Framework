...
from itak.cli import git
from itak.cli.command import BaseCommand, PlusAPIMixin
from itak.cli.config import Settings
from itak.cli.constants import DEFAULT_iTaK_ENTERPRISE_URL
from itak.cli.utils import (
...
class ToolCommand(BaseCommand, PlusAPIMixin):
    """
    A class to handle tool repository related operations for iTaK projects.
    """
    ...
    def login(self) -> None:
        ...
        if login_response.status_code != 200:
            console.print(
                "Authentication failed. Verify if the currently active organization can access the tool repository, and run 'iTaK login' again.",
                style="bold red",
            )
            try:
                console.print(
