...
from itak.llms.constants import (
    ANTHROPIC_MODELS,
    AZURE_MODELS,
    BEDROCK_MODELS,
    GEMINI_MODELS,
...
from itak.utilities import InternalInstructor
from itak.utilities.exceptions.context_window_exceeding_exception import (
    LLMContextLengthExceededError,
)
from itak.utilities.logger_utils import suppress_warnings
...
if TYPE_CHECKING:
    ...
    from itak.llms.providers.anthropic.completion import AnthropicThinkingConfig
...
class LLM(BaseLLM):
    ...
    def _get_native_provider(cls, provider: str) -> type | None:
        ...
        if provider == "openai":
            from itak.llms.providers.openai.completion import OpenAICompletion

            return OpenAICompletion

        if provider == "anthropic" or provider == "claude":
            from itak.llms.providers.anthropic.completion import (
                AnthropicCompletion,
            )
        ...
        if provider == "azure" or provider == "azure_openai":
            from itak.llms.providers.azure.completion import AzureCompletion

            return AzureCompletion

        if provider == "google" or provider == "gemini":
            from itak.llms.providers.gemini.completion import GeminiCompletion

            return GeminiCompletion

        if provider == "bedrock":
            from itak.llms.providers.bedrock.completion import BedrockCompletion
