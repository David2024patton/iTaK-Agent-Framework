...
from itak.agents.constants import FINAL_ANSWER_AND_PARSABLE_ACTION_ERROR_MESSAGE
from itak.agents.parser import (
    AgentAction,
    AgentFinish,
    OutputParserError,
    parse,
...
from itak.tools import BaseTool as iTaKTool
from itak.tools.base_tool import BaseTool
from itak.tools.structured_tool import CrewStructuredTool
from itak.tools.tool_types import ToolResult
from itak.utilities.errors import AgentRepositoryError
from itak.utilities.exceptions.context_window_exceeding_exception import (
    LLMContextLengthExceededError,
)
from itak.utilities.i18n import I18N
from itak.utilities.printer import ColoredText, Printer
...
def handle_context_length(
    ...
    if respect_context_window:
        ...
        printer.print(
            content="Context length exceeded. Consider using smaller text or RAG tools from itak_tools.",
            color="red",
        )
        raise SystemExit(
            "Context length exceeded and user opted not to summarize. Consider using smaller text or RAG tools from itak_tools."
        )
...
def _print_current_organization() -> None:
    ...
    if settings.org_uuid:
        ...
        console.print(
            "No organization currently set. We recommend setting one before using: `iTaK org switch <org_id>` command.",
            style="yellow",
        )
...
def load_agent_from_repository(from_repository: str) -> dict[str, Any]:
    ...
    if from_repository:
        ...
        from itak.cli.authentication.token import get_auth_token
        from itak.cli.plus_api import PlusAPI
        ...
        if response.status_code == 404:
            raise AgentRepositoryError(
                ...
                f"\nIf you are using the wrong organization, switch to the correct one using `iTaK org switch <org_id>` command.",
            )
...
def _setup_before_llm_call_hooks(
    ...
    if executor_context and executor_context.before_llm_call_hooks:
        from itak.hooks.llm_hooks import LLMCallHookContext
