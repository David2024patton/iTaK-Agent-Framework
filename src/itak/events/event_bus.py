"""Event bus for managing and dispatching events in iTaK.

This module provides a singleton event bus that allows registration and handling
of events throughout the iTaK system, supporting both synchronous and asynchronous
...
from itak.events.base_events import BaseEvent
from itak.events.depends import Depends
from itak.events.handler_graph import build_execution_plan
from itak.events.types.event_bus_types import (
    AsyncHandler,
    AsyncHandlerSet,
    ExecutionPlan,
    Handler,
...
from itak.events.types.llm_events import LLMStreamChunkEvent
from itak.events.utils.console_formatter import ConsoleFormatter
from itak.events.utils.handlers import is_async_handler, is_call_handler_safe
from itak.utilities.rw_lock import RWLock
...
class iTaKEventsBus:
    """Singleton event bus for handling events in iTaK.
    ...
    def __new__(cls) -> Self:
        ...
        Returns:
            The singleton iTaKEventsBus instance
        """
    ...
    def _initialize(self) -> None:
        ...
        self._sync_executor = ThreadPoolExecutor(
            max_workers=10,
            thread_name_prefix="iTaKSyncHandler",
        )
    ...
    def on(
        ...
        Example:
            >>> from itak.events import iTaK_event_bus, Depends
            >>> from itak.events.types.llm_events import LLMCallStartedEvent
            >>>
            >>> @iTaK_event_bus.on(LLMCallStartedEvent)
            >>> def setup_context(source, event):
            ...     print("Setting up context")
            >>>
            >>> @iTaK_event_bus.on(LLMCallStartedEvent, depends_on=Depends(setup_context))
    ...
    def _call_handlers(
        ...
        if errors:
            for handler, error in errors:
                self._console.print(
                    f"[iTaKEventsBus] Sync handler error in {handler.__name__}: {error}"
                )
    ...
    async def _acall_handlers(
        ...
        for handler, result in zip(handlers, results, strict=False):
            if isinstance(result, Exception):
                self._console.print(
                    f"[iTaKEventsBus] Async handler error in {getattr(handler, '__name__', handler)}: {result}"
                )
    ...
    def emit(self, source: Any, event: BaseEvent) -> Future[None] | None:
        ...
        Example:
            >>> future = iTaK_event_bus.emit(source, event)
            >>> if future:
        ...
        with self._rwlock.r_locked():
            if self._shutting_down:
                self._console.print(
                    "[iTaKEventsBus] Warning: Attempted to emit event during shutdown. Ignoring."
                )
                return None
    ...
    def scoped_handlers(self) -> Generator[None, Any, None]:
        ...
        Example:
            ...
            >>> from itak.events.event_types import CrewKickoffStartedEvent
            >>> with iTaK_event_bus.scoped_handlers():
            ...
    ...
    def shutdown(self, wait: bool = True) -> None:
        ...
        if wait:
            ...
            try:
                ...
                self._console.print(f"[iTaKEventsBus] Error waiting for tasks: {e}")
        else:
...
iTaK_event_bus: Final[iTaKEventsBus] = iTaKEventsBus()

atexit.register(iTaK_event_bus.shutdown)
