...
from itak.agents.crew_agent_executor import CrewAgentExecutor
...
from itak.memory.contextual.contextual_memory import ContextualMemory
...
from itak.utilities.agent_utils import (
    get_tool_names,
    load_agent_from_repository,
    parse_tools,
    render_text_description_and_args,
)
from itak.utilities.constants import TRAINED_AGENTS_DATA_FILE, TRAINING_DATA_FILE
from itak.utilities.converter import Converter
from itak.utilities.guardrail_types import GuardrailType
from itak.utilities.llm_utils import create_llm
from itak.utilities.prompts import Prompts, StandardPromptResult, SystemPromptResult
from itak.utilities.token_counter_callback import TokenCalcHandler
from itak.utilities.training_handler import CrewTrainingHandler


try:
    from itak.a2a.config import A2AClientConfig, A2AConfig, A2AServerConfig
except ImportError:
    A2AClientConfig = Any
    A2AConfig = Any
    A2AServerConfig = Any
...
if TYPE_CHECKING:
    from itak_tools import CodeInterpreterTool

    from itak.agents.agent_builder.base_agent import PlatformAppOrAction
    from itak.lite_agent_output import LiteAgentOutput
    from itak.task import Task
    from itak.tools.base_tool import BaseTool
    from itak.utilities.types import LLMMessage
...
class Agent(BaseAgent):
    ...
    Attributes:
            ...
            apps: List of applications that the agent can access through iTaK Platform.
    ...
    def get_platform_tools(self, apps: list[PlatformAppOrAction]) -> list[BaseTool]:
        try:
            from itak_tools import (
                iTaKPlatformTools,
            )

            return iTaKPlatformTools(apps=apps)
        except Exception as e:
    ...
    def get_mcp_tools(self, mcps: list[str | MCPServerConfig]) -> list[BaseTool]:
        """Convert MCP server references/configs to iTaK tools.
    ...
    def _get_mcp_tools_from_string(self, mcp_ref: str) -> list[BaseTool]:
        ...
        MCP references (https://... and iTaK-amp:...).
        ...
        """
        if mcp_ref.startswith("iTaK-amp:"):
    ...
    def _get_external_mcp_tools(self, mcp_ref: str) -> list[BaseTool]:
        ...
        from itak.tools.mcp_tool_wrapper import MCPToolWrapper
    ...
    def _get_native_mcp_tools(
        ...
        """
        from itak.tools.base_tool import BaseTool
        from itak.tools.mcp_native_tool import MCPNativeTool
            ...
            if mcp_config.tool_filter:
                ...
                for tool in tools_list:
                    if callable(mcp_config.tool_filter):
                        try:
                            from itak.mcp.filters import ToolFilterContext
    ...
    def _get_amp_mcp_tools(self, amp_ref: str) -> list[BaseTool]:
        """Get tools from itak AMP MCP marketplace."""
        # Parse: "iTaK-amp:mcp-name" or "iTaK-amp:mcp-name#tool_name"
        amp_part = amp_ref.replace("iTaK-amp:", "")
        if "#" in amp_part:
    ...
    def _fetch_amp_mcp_servers(mcp_name: str) -> list[dict[str, Any]]:
        """Fetch MCP server configurations from itak AMP API."""
    ...
    def get_multimodal_tools() -> Sequence[BaseTool]:
        from itak.tools.agent_tools.add_image_tool import AddImageTool
    ...
    def get_code_execution_tools(self) -> list[CodeInterpreterTool]:
        try:
            ...
            self._logger.log(
                "info", "Coding tools not available. Install iTaK_tools. "
            )
            return []
